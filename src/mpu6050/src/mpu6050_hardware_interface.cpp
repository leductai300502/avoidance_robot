#include "mpu6050/mpu6050_hardware_interface.hpp"
...
return_type MPU6050Hardware::read(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
	...
	angular_vel_x = (double)gyro_values[0];	
	angular_vel_y = (double)gyro_values[1];	
	angular_vel_z = (double)gyro_values[2];	

	linear_accel_x = (double)accel_values[0];
